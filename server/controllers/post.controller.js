...
class PostController {
    findAllPosts(req,res){
      ...
      if (tag == undefined){
        ...
        var tags = tag.split(",")
        const route = tags.map((tag,t)=> {
          let qPosts = posts.posts.filter(post => post.tags.includes(tag))
          res.json({results: qPosts})
        })
      }else{
        let qPosts = posts.posts.filter(post => post.tags.includes(tag))
        res.json({results: qPosts})
    ...
    //         articles: articles
    // });
        // Post.find() //these are mongoose built-in methods
        //     .then(allPosts =>{
